...
export async function POST(req: NextRequest) {
  try {
    ...
    // Use locale from payload (validated), fallback to request detection
    const requestLang = getRequestLang(req)
    const lang = (locale === 'fa' || locale === 'en') ? locale : requestLang
...
function generateAdminEmailHTML(data: {
  ...
  const safeBudgetRange = escapeHtml(data.budgetRange)
    ...
    <div style="background-color: #1a1a1a; border: 1px solid #2a2a2a; border-radius: 12px; padding: 32px; margin-bottom: 24px;">
      <div style="margin-bottom: 24px;">
        ...
        <div style="background-color: #0f0f0f; border-radius: 8px; padding: 16px;">
          <p style="margin: 8px 0; font-size: 14px; color: #e5e5e5; text-align: ${textAlign};">
            <strong style="color: #ffffff;">${data.locale === 'fa' ? 'نام:' : 'Name:'}</strong> ${safeName}
          </p>
          <p style="margin: 8px 0; font-size: 14px; color: #e5e5e5; text-align: ${textAlign};">
      ...
      <div style="margin-bottom: 24px;">
        ...
        <div style="background-color: #0f0f0f; border-radius: 8px; padding: 16px;">
          ...
          <p style="margin: 8px 0; font-size: 14px; color: #e5e5e5; text-align: ${textAlign};">
            <strong style="color: #ffffff;">${data.locale === 'fa' ? 'نوع پروژه:' : 'Project Type:'}</strong> ${safeProjectType}
          </p>
          <p style="margin: 8px 0; font-size: 14px; color: #e5e5e5; text-align: ${textAlign};">
            <strong style="color: #ffffff;">${data.locale === 'fa' ? 'بودجه:' : 'Budget Range:'}</strong> ${safeBudgetRange}
          </p>
          <p style="margin: 8px 0; font-size: 14px; color: #e5e5e5; text-align: ${textAlign};">
            <strong style="color: #ffffff;">${data.locale === 'fa' ? 'ددلاین:' : 'Deadline:'}</strong> ${safeDeadline}
          </p>
        </div>
      </div>
      ...
      <div style="margin-bottom: 24px;">
        ...
        <div style="background-color: #0f0f0f; border-radius: 8px; padding: 16px;">
          <p style="margin: 0; font-size: 14px; color: #e5e5e5; text-align: ${textAlign}; white-space: pre-wrap;">${safeMessage}</p>
        </div>
      </div>
      ...
      <div style="margin-top: 24px; padding-top: 20px; border-top: 1px solid #2a2a2a; font-size: 12px; color: #808080; text-align: ${textAlign};">
        ...
        <p style="margin: 4px 0;"><strong>${data.locale === 'fa' ? 'URL:' : 'URL:'}</strong> ${safeUrl || 'N/A'}</p>
...
function generateUserConfirmationHTML(name: string, locale: 'fa' | 'en'): string {
  ...
  const greeting = locale === 'fa' ? `سلام ${safeName}،` : `Hi ${safeName},`
